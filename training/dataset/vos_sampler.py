...
## multi-time split
class RandomUniformSampler_MultiTime(VOSSampler):
    def __init__(
        self,
        num_frames,
        max_num_objects,
        ...
        self.max_num_objects = max_num_objects
        self.reverse_time_prob = reverse_time_prob
        self.choice_num = choice_num  # 0: [1,2], 1: [1,3], 2: [1,2,3]

    def sample(self, video, segment_loader, epoch=None):
        # setting for stride choices and probabilities
        # stride_choices = [1, 2, 3]
        # stride_probs = [0.5, 0.3, 0.2]
        ## used
        # stride_choices = [1, 2]
        # stride_probs = [0.9, 0.1]

        if self.choice_num == 0:
            stride_choices = [1, 2]
            stride_probs = [0.9, 0.1]
        elif self.choice_num == 1:
            stride_choices = [1, 3]
            stride_probs = [0.9, 0.1]
        elif self.choice_num == 2:
            stride_choices = [1, 2, 3]
            stride_probs = [0.8, 0.1, 0.1]


        for retry in range(MAX_RETRIES):
            # randomly select a stride based on defined probabilities
            stride = random.choices(stride_choices, weights=stride_probs, k=1)[0]

            # calculate the maximum possible start index
            max_start = len(video.frames) - (self.num_frames - 1) * stride
            if max_start <= 0:
                raise Exception(
                    f"Cannot sample {self.num_frames} frames with stride {stride} from video {video.video_name}."
                )

            start = random.randrange(0, max_start)
            frames = [video.frames[start + i * stride] for i in range(self.num_frames)]

            # randomly reverse the order of frames
            if random.uniform(0, 1) < self.reverse_time_prob:
                frames = frames[::-1]

            # object ids in the first frame
            visible_object_ids = []
            loaded_segms = segment_loader.load(frames[0].frame_idx)
            if isinstance(loaded_segms, LazySegments):
                visible_object_ids = list(loaded_segms.keys())
