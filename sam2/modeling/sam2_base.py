...
from sam2.modeling.cal_similarity_sp import euclidean_similarity_sp, cosine_similarity_sp, dot_product_similarity_sp, \
      pearson_correlation_sp, manhattan_similarity_sp, ssim_channelwise_sp, spearman_rank_correlation_sp, spatial_similarity_sp
...
def prune_memory_features_time_split(maskmem_features, maskmem_pos_enc, total_prune_frame_num=2, prune_sim="cosine"):
    ...
    def compute_similarity(candidates, reference, mode):
        ...
        if mode == "cosine":
            ...
            sim = cosine_similarity_sp(candidates_stacked, reference_expanded)
        ...
        elif mode == "spearman_sp":
            sim = spearman_rank_correlation_sp(candidates_stacked, reference_expanded)
        elif mode == "spatial_sp":
            sim = spatial_similarity_sp(candidates_stacked, reference_expanded)
        else:
...
class SAM2Base(torch.nn.Module):
    def __init__(
        ...
        use_mem_split: bool = False,  # ap: split memory into two parts, one for the short-term frame and one for the long-term frames
        use_prune: bool = False,  # ap: prune the memory by cosine similarity
        prune_sim = "cosine_sp",  # ap: cosine similarity
    ):
        super().__init__()
        ...
        self.use_mem_split = use_mem_split  # ap: split memory into two parts, one for the short-term frame and one for the long-term frames
        self.use_prune = use_prune  # ap: prune the memory by cosine similarity
        self.prune_sim = prune_sim  # ap: cosine similarity
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame:
            ...
            if not self.use_prune:
                ...
                if (len(to_cat_memory) + self.num_frame_to_prune) > self.num_maskmem: # ap
                    if not self.use_mem_split:
                        ...
                        # ap: use spatial cosine similarity
                        similarities = cosine_similarity_sp(candidate_frames, last_frame_expanded)  # [num_frames-2, 64, bs]
                        similarities = torch.sum(similarities, dim=1)  # [num_frames-2, bs]
                        ...
                        to_cat_memory, to_cat_memory_pos_embed = prune_memory_features_time_split(to_cat_memory, to_cat_memory_pos_embed, 
                                                                                            total_prune_frame_num=2, prune_sim=self.prune_sim) # "euclidean_sp" self.prune_sim
